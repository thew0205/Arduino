...
class DMDFrameDoubleBuffer
{
 ...
 public:
  ...
  // DMDFrameDoubleBuffer subFrame(unsigned int left, unsigned int top, unsigned int width, unsigned int height);

  // // Copy the contents of another frame back into this one at the given location
  // void copyFrame(DMDFrameDoubleBuffer &from, unsigned int left, unsigned int top);
...
public:
  DMD_TextBox(DMDFrameDoubleBuffer &dmd, int left = 0, int top = 0, int width = 0, int height = 0);
